...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        """
        # init values
        homo_type = model_kwargs['homo_type']
        task_nam = model_kwargs['task_nam']
        enable_er = model_kwargs['enable_er']
        pad_token_id = generation_config.pad_token_id
        output_attentions = generation_config.output_attentions
        output_hidden_states = generation_config.output_hidden_states
        output_scores = generation_config.output_scores
            ...
            if model_inputs['past_key_values'] is not None:
                ...
                for i in range(len(kv_cache)):
                    for j in range(len(kv_cache[i])):
                        num_bytes = kv_cache[i][j].numel() * kv_cache[i][j].element_size()
                        num_mb = num_bytes / (1024 ** 2)
                        kv_cache_size += num_mb
        ...
        if streamer is not None:
            streamer.end()
        decode_time = decode_time / len(input_ids[0])
        
        # Case1: ER+Ours
        if enable_er and homo_type == 2:
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_ours_with_er_prefill.txt', 'a') as f:
                f.write(f'Prefill: {prefill_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_ours_with_er_decode.txt', 'a') as f:
                f.write(f'Decode: {decode_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_ours_with_er_kvcache.txt', 'a') as f:
                f.write(f'kvcache: {kv_cache_size}\n')
        elif enable_er and homo_type == 0:
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_er_without_ours_prefill.txt', 'a') as f:
                f.write(f'Prefill: {prefill_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_er_without_ours_decode.txt', 'a') as f:
                f.write(f'Decode: {decode_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_er_without_ours_kvcache.txt', 'a') as f:
                f.write(f'kvcache: {kv_cache_size}\n')
        elif homo_type == 1:
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_static_prefill.txt', 'a') as f:
                f.write(f'Prefill: {prefill_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_static_decode.txt', 'a') as f:
                f.write(f'Decode: {decode_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_static_kvcache.txt', 'a') as f:
                f.write(f'kvcache: {kv_cache_size}\n')
        elif enable_er == False and homo_type == 0:
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_full_prefill.txt', 'a') as f:
                f.write(f'Prefill: {prefill_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_full_decode.txt', 'a') as f:
                f.write(f'Decode: {decode_time}\n')
            with open(f'/root/LLaVA-NeXT/exp_res/{task_nam}_full_kvcache.txt', 'a') as f:
                f.write(f'kvcache: {kv_cache_size}\n')
        ...
        print(f'Prefill: {prefill_time}s')
        print(f'Decode: {decode_time}s')
        print(f'KVCache: {kv_cache_size}MB')
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                return GenerateEncoderDecoderOutput(
                    sequences=input_ids,
